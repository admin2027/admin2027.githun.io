...
<div id="body">
    <el-container>
      ...
      <a v-show="showname==false" href="#" @click="userlogin()">用户登录</a> 
      <a v-show="showname==true" href="#" @click="enterbookshelf()">欢迎{{userName}}</a>
      <a v-show="showname==true" href="#" @click="enterbookshelf()">个人书架</a>
      <a v-show="showname==true" href="#" @click="enterbookshelf()" style="padding-top:0px">
      <img  :src="userImage" calss="avatar_box" alt="">
      </a>
      <a v-show="showname==true"  href="#"   @click="quit()">退出登录</a>


      <a href="#" @click="managerlogin()">管理员登录</a> 
      ...
      <div class="header">
        ...
        <el-input style="width:320px;margin-right:10px" v-model="searchBook"> </el-input>
        <el-button type="primary" icon="el-icon-search"  @click="searchbook()">搜索</el-button>
        </div>
      </div>

      <div class="nav">
        <!-- <div style=" position:absolote;float:right"> -->
        <ul translate="">
          <li><a href="">首页</a></li>
...
export default {
  data() {
    ...
    classboook(item){
      console.log(item.id)
      this.$router.push({path:`/class/${item.class.id}`})
    },
    //进入个人中心
    enterbookshelf(){
      this.$router.push({name:"shelfbook",params:{name:this.userName}})
    },
    //搜索按钮
    searchbook(){ 
      this.$router.push({name:"searchBook",params:{name:this.searchBook}})
    },
    async getclass(){
